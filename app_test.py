...
class TestCronExpression(unittest.TestCase):
    ...
    def test_init(self):
        cron = CronExpression("0 0 1 1 1 /usr/bin/find")
        ...
        self.assertEqual(cron.command, "/usr/bin/find")
    
    def test_init_with_complex_expression(self):
        cron = CronExpression("*/5 0-12 1,15 */2 1-5 /usr/bin/find")
    ...
    def test_to_table_format(self):
        ...
        self.assertEqual(table[5], ("command", "/usr/bin/find"))
...
class TestRawCronExpression(unittest.TestCase):
    ...
    def test_init(self):
        cron = RawCronExpression("0 0 1 1 1 /usr/bin/find")
        self.assertEqual(cron.minute, "0")
        self.assertEqual(cron.hour, "0")
        self.assertEqual(cron.day_of_month, "1")
        self.assertEqual(cron.month, "1")
    ...
    def test_expand(self):
        raw = RawCronExpression("*/5 0-12 1,15 */2 1-5 /usr/bin/find")
        expanded = raw.expand()
        ...
        self.assertEqual(expanded.command, "/usr/bin/find")
...
class TestExpandCronExp(unittest.TestCase):
    ...
    def test_simple_expression(self):
        result = expand_cron_exp("0 0 1 1 1 /usr/bin/find")
        expected = "minute         0\nhour           0\nday of month   1\nmonth          1\nday of week    1\ncommand        /usr/bin/find"
        self.assertEqual(result, expected)
    
    def test_complex_expression(self):
        result = expand_cron_exp("*/5 0-12 1,15 */2 1-5 /usr/bin/find")
        ...
        self.assertIn("command        /usr/bin/find", result)
    
    def test_command_with_arguments(self):
        result = expand_cron_exp("0 0 1 1 1 /usr/bin/find -a -b --c=d")
        self.assertIn("command        /usr/bin/find -a -b --c=d", result)
...
class TestMain(unittest.TestCase):
    ...
    @patch('sys.argv', ['app.py', '0 0 1 1 1 /usr/bin/find'])
    @patch('sys.stdout', new_callable=StringIO)
    def test_main_success(self, mock_stdout):
        from app import main
        main()
        self.assertIn("minute         0", mock_stdout.getvalue())
        self.assertIn("command        /usr/bin/find", mock_stdout.getvalue())
